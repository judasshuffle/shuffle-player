...
class MPVClient:
    ...
    def load_playlist(self, paths: Sequence[str], clear_first: bool = True) -> bool:
        # Hard reset so we immediately switch away from whatever is currently playing
        self.command("stop")
        ...
        for p in paths:
            ...
            if not first_loaded:
                resp = self.command("loadfile", p, "replace")
                first_loaded = (resp.get("error") == "success")
                ok_any = ok_any or first_loaded
            else:
...
def parse_intent(transcript: str, artists: List[str]) -> ParsedCommand:
    ...
    if not has_trigger:
        # If the whole utterance matches an artist, treat it as "play <artist>"
        a0 = best_match_artist(t, artists)
        if a0 and len(a0.strip()) >= 3:
            cmd = ParsedCommand(intent="play")
            cmd.artist = a0
            return cmd

        # Otherwise ignore unrelated speech so we don't constantly reload playlists.
