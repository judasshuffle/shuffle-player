...
def record_audio():
    PIN = os.environ.get("VOICE_PTT_PIN", "17")
    CHIP = os.environ.get("VOICE_PTT_CHIP", "gpiochip0")
    DEBOUNCE = os.environ.get("VOICE_PTT_DEBOUNCE", "200ms")

    print(f"PTT: waiting on GPIO{PIN} (press & hold to talk)...", flush=True)
    subprocess.run(["./bin/wait_edge.sh", PIN, CHIP, "falling", DEBOUNCE], check=True)

    print("PTT: Recording... (release to stop)", flush=True)
    rec = subprocess.Popen(ARECORD_CMD, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    subprocess.run(["./bin/wait_edge.sh", PIN, CHIP, "rising", DEBOUNCE], check=True)
